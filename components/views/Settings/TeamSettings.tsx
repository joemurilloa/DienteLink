    ...
    <div className="card-premium p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">Invita a secretarias y asistentes para colaborar</p>
        </div>
      </div>

        {/* Invite Form */}
        <div className="bg-slate-50 border border-slate-300 rounded-2xl p-5 mb-8">
          ...
          <form onSubmit={handleInvite} className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              <input
                ...
                className="w-full bg-white px-4 py-2.5 rounded-xl border border-slate-300 text-sm outline-none focus:border-indigo-500 transition-all placeholder:text-slate-500"
              />
            </div>
        ...
        {invitations.length > 0 && (
          <div className="mb-8">
            <h4 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-3">Invitaciones Pendientes</h4>
            <div className="space-y-2">
              {invitations.map(inv => (
                <div key={inv.id} className="flex items-center justify-between p-3.5 bg-white border border-slate-300 rounded-xl shadow-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">Rol: {getRoleLabel(inv.role)}</p>
                    </div>
                  </div>
                  <button
        ...
        {/* Active Team Members */}
        <div>
          <h4 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-3">Miembros Activos</h4>
          {loading ? (
            <p className="text-sm text-slate-500 animate-pulse">Cargando equipo...</p>
          ) : (
            <div className="space-y-2">
              {teamMembers.map(member => {
                const isOwner = member.role === 'owner';
                return (
                  <div key={member.id} className="flex items-center justify-between p-3.5 bg-white border border-slate-300 rounded-xl shadow-sm">
                    <div className="flex items-center gap-3">
                      <div className={`w-11 h-11 rounded-full flex items-center justify-center text-white text-sm font-bold ${isOwner ? 'bg-indigo-500' : 'bg-slate-300'}`}>
