...
const Sidebar: React.FC<SidebarProps> = React.memo(({ activePath, pendingRequestsCount = 0 }) => {
  ...
  return (
    <aside className="hidden md:flex flex-col w-[72px] lg:w-[280px] h-full sidebar-dark p-3 lg:p-6 relative overflow-hidden transition-all">
      {/* Subtle glow */}
      ...
      {/* Logo */}
      <div className="flex items-center gap-3.5 mb-12 px-0 lg:px-3 cursor-pointer group relative z-10 justify-center lg:justify-start" onClick={() => navigate('/')}>
        ...
        <div>
          <h1 className="text-lg font-extrabold text-white tracking-tight leading-none hidden lg:block">DienteLink</h1>
          <p className="text-[10px] font-semibold text-blue-400/80 uppercase tracking-widest mt-0.5 hidden lg:block">Gestión Dental</p>
        </div>
      </div>
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "w-full flex items-center justify-center lg:justify-between gap-0 lg:gap-3 px-0 lg:px-4 py-3 rounded-xl transition-all duration-200 font-semibold text-sm tracking-tight relative group",
                isActive
      ...
      <div className="relative z-10 mt-auto">
        <div className="p-2 lg:p-3 bg-white/5 rounded-2xl border border-white/5">
          <div className="flex items-center gap-3 cursor-pointer group justify-center lg:justify-start" onClick={() => navigate('/settings')}>
            ...
            <Settings size={14} className="text-slate-600 group-hover:text-slate-400 group-hover:rotate-90 transition-all duration-300 hidden lg:block" />
          </div>
        </div>
      </div>
    </aside>
