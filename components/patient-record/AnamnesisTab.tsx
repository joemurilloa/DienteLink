    ...
    return (
        <div className="space-y-8 animate-in-up duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="p-5 bg-white rounded-2xl border border-slate-300">
                    <div className="flex items-center gap-2.5 mb-3">
                        <div className="w-11 h-11 bg-slate-700 text-white rounded-xl flex items-center justify-center"><User size={16} /></div>
                        <label className="text-sm font-semibold uppercase tracking-wider text-slate-500">Antecedentes Familiares</label>
                    </div>
                    <div className="flex flex-wrap gap-1.5 mb-3">
                        {COMMON_FAMILY.map(f => (
                            <button key={f} type="button"
                                onClick={() => appendToField('familyHistory', f)}
                                className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-slate-50 text-slate-500 border border-slate-300 hover:bg-slate-700 hover:text-white hover:border-slate-700 transition-all active:scale-95">
                                + {f}
                            </button>
                        ))}
                    </div>
                ...
                <div className="p-5 bg-white rounded-2xl border border-slate-300">
                    <div className="flex items-center gap-2.5 mb-3">
                        <div className="w-11 h-11 bg-indigo-500 text-white rounded-xl flex items-center justify-center"><Activity size={16} /></div>
                        <label className="text-sm font-semibold uppercase tracking-wider text-indigo-500">Hábitos</label>
                    </div>
                    <div className="flex flex-wrap gap-1.5 mb-3">
                        {COMMON_HABITS.map(h => (
                            <button key={h} type="button"
                                onClick={() => appendToField('habits', h)}
                                className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-indigo-50 text-indigo-500 border border-indigo-200 hover:bg-indigo-500 hover:text-white hover:border-indigo-500 transition-all active:scale-95">
                                + {h}
                            </button>
                        ))}
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-4 bg-white rounded-2xl border border-slate-300">
                    <label className="text-sm font-semibold uppercase tracking-wider text-slate-500 mb-2 block">Tipo de Sangre</label>
                    <select
                        value={hist.bloodType || ''}
                        onChange={(e) => onUpdate({ ...patient, clinicalHistory: { ...hist, bloodType: e.target.value } })}
                        className="w-full px-3 py-2 rounded-xl border border-slate-300 text-sm font-medium text-slate-900 outline-none focus:border-blue-500 transition-all bg-white"
                    >
                        <option value="">No registrado</option>
                        {['A+','A-','B+','B-','AB+','AB-','O+','O-'].map(t => <option key={t} value={t}>{t}</option>)}
                    </select>
                </div>
                <div className="p-4 bg-white rounded-2xl border border-slate-300 flex items-center justify-between">
                    <div>
                        <label className="text-sm font-semibold uppercase tracking-wider text-slate-500 block">Fumador</label>
                ...
                <div className="p-4 bg-white rounded-2xl border border-slate-300 flex items-center justify-between">
                    <div>
                        <label className="text-sm font-semibold uppercase tracking-wider text-slate-500 block">Embarazo</label>
            ...
            <div className="p-5 bg-amber-50 rounded-2xl border border-amber-100">
                <div className="flex items-center gap-2.5 mb-3">
                    <div className="w-11 h-11 bg-amber-500 text-white rounded-xl flex items-center justify-center"><ClipboardList size={16} /></div>
                    <label className="text-sm font-semibold uppercase tracking-wider text-amber-600">Observaciones Generales</label>
                </div>
                <DebouncedTextarea
            ...
            <div className="p-5 bg-blue-50 rounded-2xl border border-blue-100">
                <div className="flex items-center gap-2.5 mb-4">
                    <div className="w-11 h-11 bg-blue-600 text-white rounded-xl flex items-center justify-center"><ClipboardList size={16} /></div>
                    <label className="text-sm font-semibold uppercase tracking-wider text-blue-600">Motivo de Consulta Principal</label>
                </div>
                <div className="flex flex-wrap gap-1.5 mb-3">
                    {COMMON_MOTIVES.map(m => (
                        <button key={m} type="button"
                            ...
                            className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-white text-blue-500 border border-blue-200 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all active:scale-95">
                            + {m}
                        </button>
                    ))}
                </div>
